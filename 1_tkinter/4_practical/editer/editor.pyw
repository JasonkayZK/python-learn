...
class EditorPlus(Tk):
    ...
    def _create_body_(self):
        ...
        self.content_text.pack(expand=1, fill='both')
    ...
    # 响应快捷菜单
    def _shortcut_action(self, ttype):
        def handle():
            if ttype == "new_file":
                self.new_file()
            elif ttype == "open_file":
                self.open_file()
            elif ttype == "save":
                self.save()
            elif ttype == "cut":
            ...
            elif ttype == "find_text":
                self.find_text()

            if ttype != "copy" and ttype != "save":
                self._update_line_num()
